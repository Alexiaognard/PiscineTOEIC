...
def create_subject_prof(request):
    if request.session['estEtu']:
        ...
        if request.method == 'POST':
            ...
            if form.is_valid():
                sub=Sujet(nomSujet=form.cleaned_data["nomSujet"],mdpSujet=form.cleaned_data["mdpSujet"],noteSujet=0)
                sub.save()
                print("prof",sub)
    ...
    if not request.session['estEtu']:
        ...
        if request.method == 'POST':
            if form.is_valid():
                ...
                sub = Sujet(nomSujet=subProf.nomSujet, mdpSujet=subProf.mdpSujet, noteSujet=0)
                sub.save()
...
@login_required
def stats_par_partie_etu(request):
    if  not request.session['estEtu']:
        return render(request, 'error404.html')
    else:
